...
RUN set -ex; \
    ...
    \
    # 编译安装 pg_lake
    git clone https://github.com/Vonng/pg_lake.git /tmp/pg_lake && \
    cd /tmp/pg_lake && \
    ...
    apt-get clean && \
    rm -rf /tmp/pigsty-key /etc/apt/keyrings/pigsty.gpg /etc/apt/sources.list.d/pigsty-io.list /var/lib/apt/lists/* /tmp/postgresbson
